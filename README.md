...
- `GET /api/status` – runtime status (mode, paused, kill switch, latencies). Always carries `sessionId`, `sessionStartedAt`, `session {id, startedAt, mode, initialEquityUsd}` and `pnl` (canonical snapshot or `null`); the same builder feeds every WS `StatusUpdate`.
- `GET /api/pnl` – canonical PnL / equity snapshot. **Equity single source of truth: `totalEquityUsd`** (also at `/api/status.pnl.totalEquityUsd`); session anchor `sessionStartEquityUsd` = `trading_sessions.initial_equity` (paper: `guardrails.account.equity_usd`).
- `GET /api/orders|fills|signals|positions?session_id=&execution_mode=&limit=` (+`status=open|closed|all` on positions) – session-scoped Supabase reads. Default = active session; response `{ <table>: rows, count, scope }` where `scope.filter` is `session_id` (migration `20260911170000` applied), `time_window` (interim `<time col> >= sessionStartedAt`) or `none` (no session → empty). `/api/positions` for the active session adds `engineOpenPositions`.
- `GET /api/analytics/session|equity-curve|trades` – active session only (400 when the engine is stopped); carry `sessionId` / `sessionStartedAt`.
- `GET /api/analytics/strategies[?session_id=]` – session-scoped per-strategy trade stats: `closedTrades`, `openTrades`, `wins/losses/breakeven`, `winRate|null`, `realizedPnlUsd`, `pnlToday`, `signalsGenerated` (signal counter, not trades). Also mirrored as `sessionStats` on each `GET /api/strategies` entry.
