...
describe('committed multi-TF fixtures are real, contiguous and self-describing', () => {
  function assertContiguous(file: BarFixtureFile, step: number): void {
    for (let i = 0; i < file.candles.length; i++) {
      const c = file.candles[i];
      expect(c.time % step).toBe(0);
      if (i > 0) expect(c.time - file.candles[i - 1].time).toBe(step);
  ...
  it('1d/: BTC, ETH, SOL — 2024-09-01 → 2026-08-31, 730 native ONE_DAY bars', () => {
    for (const symbol of TRIO) {
      const file = JSON.parse(fs.readFileSync(path.join(FIXTURES, '1d', `${symbol}.json`), 'utf8')) as BarFixtureFile;
      expect(file.symbol).toBe(symbol);
      expect(file.source).toBe('coinbase-advanced-trade-public');
  ...
  it('4h rolled to 1d reproduces the native 1d OHLC for August 2026 (internal consistency)', () => {
    for (const symbol of TRIO) {
      ...
      for (const day of rolled.candles) {
        ...
        // Coinbase's own volume differs slightly across granularities (see MULTI_TF.md); OHLC is exact.
