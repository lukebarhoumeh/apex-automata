...
--
-- Idempotency note (W2 D11, 2026-05-14): the schema gap was reconciled live via
-- 20260514210000_meta_filter_decisions_schema_restore_w2_d11. This file is the
-- canonical CREATE-from-scratch DDL used by fresh-clone bring-up and Preview
-- branches. user_id is NULLABLE (not NOT NULL as originally drafted) because the
-- single-tenant bot insert at atlas/apps/core-node/src/strategies/meta-filter.ts
-- intentionally omits the field. All CREATE INDEX/POLICY statements are guarded
-- with IF NOT EXISTS / DROP POLICY IF EXISTS so re-application is safe.
...
-- NOTE: with user_id NULLABLE, rows inserted without user_id will not match
-- auth.uid() and remain accessible only via service-role / non-RLS contexts.
-- This matches the single-tenant bot model where the trading runtime writes via
-- service-role and there are no logged-in users hitting this table directly.
