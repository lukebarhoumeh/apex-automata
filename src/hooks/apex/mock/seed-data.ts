/**
 * Seeded DEMO fixtures for the pages that are not yet wired to the runtime:
 * Model, Backtest, Journal, Alerts, Settings. Every consumer renders a
 * DemoBanner and carries a DEMO nav badge (see shell/nav-config.ts).
 *
 * Paper-SoT surfaces (Dashboard, Orders, Signals, Risk) MUST NOT import from
 * this module. The session / positions / signals / orders / strategy /
 * regime / risk fixtures that used to live here were deleted so nothing can
 * render "Breakout ACCEPTED", a $100k equity curve or seeded fills as if they
 * belonged to the active paper session.
 */
