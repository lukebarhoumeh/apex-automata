...
export function EngineControls() {
  const { engineRunning, mode, sessionId } = useActiveSession();
      ...
      <div
        className={cn(
          ...
          engineRunning
            ...
            : "border-obsidian-line bg-obsidian-2 text-fg-2",
        )}
      ...
      >
        <span
          className={cn(
            ...
            engineRunning
              ? "bg-up shadow-[0_0_0_2px_hsl(var(--up)/0.25)]"
              : "bg-fg-3",
          )}
        />
        {engineRunning ? `${mode ?? "paper"} · running` : "stopped"}
      </div>
